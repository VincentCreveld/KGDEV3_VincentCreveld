...
public class DungeonGrid : MonoBehaviour {
	...
	{
		return new IntVector2((int)dungeonSize,(int)dungeonSize);
	}

	//This function is the main function that generated the dungeon.
	[ContextMenu("Generate Dungeon")]
	private void GenerateDungeon() {
		dungeonGrid = new Tile[(int)dungeonSize, (int)dungeonSize];
		doors = new GameObject[(int)dungeonSize, (int)dungeonSize];
		PlaceRooms();
		PlacePaths();
		CheckRoomsConnected();
		CleanUpRoomBorders();
	...
	private void SetupGrid(Room r) {
		for(int i = r.xPos - roomMargin; i < r.xPos + r.xSize + roomMargin; i++) {
			for(int j = r.yPos - roomMargin; j < r.yPos + r.ySize + roomMargin; j++) {
				if(i >= 0 && j >= 0 && i <= (int)dungeonSize - 1 && j <= (int)dungeonSize - 1)
	...
	private IntVector2 getXY() {
		return new IntVector2(UnityEngine.Random.Range(0, (int)dungeonSize- 1 - (roomXRange.max * 2)), UnityEngine.Random.Range(0, (int)dungeonSize - 1 - (roomYRange.max * 2)));
	}
	...
	private void GenerateGrid() {
		ClearGrid();
		for(int i = 0; i < (int)dungeonSize; i++) {
			for(int j = 0; j < (int)dungeonSize; j++) {
